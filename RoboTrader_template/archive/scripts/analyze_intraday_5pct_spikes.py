"""
analyze_intraday_5pct_spikes.py
================================
2026년 4~5월 하루 종일 윈도우(09:00~15:30) 내에
running-min-low 대비 이후 고점이 5% 이상 상승한 케이스를 추출합니다.

"5% 이상 상승" 정의:
  - 09:00~15:30 윈도우에서 각 분봉마다 그 시점까지의 누적 최저가(running_min_low) 추적
  - 그 이후 any 분봉의 high / running_min_low >= 1.05 이면 포착
  - low_time이 high_time 이전이어야 하는 조건 유지

출력:
  - CSV: RoboTrader_template/reports/intraday_5pct_spikes_aprmay/spikes.csv
  - 콘솔 요약 (총 케이스, 시간대별 도달 분포, 상위 30개 등)

사용법:
  cd RoboTrader_template
  python scripts/analyze_intraday_5pct_spikes.py
"""
...
REPORT_DIR = PROJECT_ROOT / "reports" / "intraday_5pct_spikes_aprmay"
...
# Step 1: 케이스별 상세 추출 (running min + peak ratio + 저점/고점 시각)
# time 컬럼은 'HHMMSS' 형식이므로 문자열 비교로 필터
# 전체 정규장 09:00~15:30 (종가 포함)
SQL_DETAILS = """
WITH win AS (
    SELECT
        stock_code,
...
-- 09:00 시가(첫 분봉 open)와 15:30 직전 마지막 종가(close_eod)
open_close AS (
    SELECT
        stock_code,
        trade_date,
...
# Step 3: 일별 전체 시가/종가 (minute_candles 기준)
SQL_DAY_OHLC = """
SELECT
    stock_code,
    trade_date,
...
def time_bucket(hhmm: str) -> str:
    """'HH:MM' 형식의 high_time을 시간대 구간으로 분류"""
...
def main() -> None:
    ...
    print("=" * 70)
    print("  장중 5% 상승 케이스 추출 (2026-04 ~ 2026-05, 09:00~15:30 전체)")
    print("=" * 70)
    ...
    name_map: dict = {row[0]: row[1] for row in cur.fetchall()}
    ...
    for row in rows:
        ...
        records.append({
            ...
            "close_eod": round(float(close_eod), 0) if close_eod else "",
    ...
    # 시간대별 도달 분포 (high_time 기준)
    BUCKETS_ORDER = ["09~10시", "10~11시", "11~12시", "12~13시", "13~14시", "14~15시", "15~15:30", "기타"]
    time_dist: Counter = Counter()
    for r in records:
        ht = r.get("high_time", "")
        time_dist[time_bucket(ht)] += 1

    print("  [시간대별 고점 도달 분포 (오전 vs 오후 비교)]")
    max_t = max(time_dist.values()) if time_dist else 1
    for label in BUCKETS_ORDER:
        cnt = time_dist.get(label, 0)
        pct_share = cnt / total_cases * 100 if total_cases else 0
        b = bar(cnt, max_t, width=20)
        print(f"    {label:<10}  {b}  {cnt:5,}건  ({pct_share:5.1f}%)")
    print()
