"""Elder Triple Screen 일봉 백테스트.

usage:
  python scripts/run_elder_triple_screen.py --variant A --all-modes
  python scripts/run_elder_triple_screen.py --variant B --mode single --rule triple_screen_stochastic

데이터: daily_prices (adj_factor 적용 수정주가)
universe: top_volume:50 (일평균 거래대금 상위 50)
Screen 1: 종목 자기완결 (일봉 65일 EMA proxy) — 지수/RS 불필요
Screen 3: 매수스톱 (전일 고가 +1틱, 최대 2일 추적) — run 스크립트가 처리
청산: Variant A (sl 8% / tp 30% / mh 100 / EMA13 trail + ema65 반전) 또는 B (sl 8% / tp 12% / mh 20)
"""
...
from strategies.books.elder_triple_screen.rules import ALL_RULES, ema, krx_tick, screen1_uptrend
from strategies.books.elder_triple_screen.strategy import BOOK_META, build_strategy

LOG = logging.getLogger("elder_triple_screen")

# Screen 3 매수스톱 추적 최대 일수 (신호 발생 다음날 포함 N_TRAIL일 동안 체결 시도)
N_TRAIL = 2

VARIANT_PARAMS = {
    "A": dict(stop_loss_pct=0.08, take_profit_pct=0.30, max_hold_bars=100, trail_ema=13, trend_flip_exit=True),
    "B": dict(stop_loss_pct=0.08, take_profit_pct=0.12, max_hold_bars=20, trail_ema=None, trend_flip_exit=False),
}
...
def _load_top_volume_universe(start: str, end: str, top_n: int = 50) -> List[str]:
    ...
    종목처럼 섞여 있고 close*volume 이 압도적이라 필터 없이는 top-50 중 3자리
    (KOSPI 1위·KS11 5위·KQ11 6위)를 지수가 차지한다. 상세 → lib/universe_filter.py
    """
...
def simulate_one_stock(
    ...
    신호 t → Screen 3 매수스톱(전일 고가+1틱, 최대 N_TRAIL일 추적) → sl/tp/mh/EMA13 trail/추세반전 청산.
    no-lookahead: rule은 df[:i+1]만, 체결 판정은 각 bar의 OHLC만 사용.
    """
    ...
    # Screen 3 대기 매수스톱: {"trigger_high_idx": j, "days_left": k}
    # trigger 가격은 j(=직전 봉)의 고가 +1틱. 매일 갱신(직전 봉 고가로).
    ...
    for i in range(warmup_bars, n - 1):
        ...
        # Screen 3 매수스톱 처리 (대기 신호가 있으면 bar_next OHLC로 체결 판정)
        if position is None and pending is not None:
            prior_high = float(df.iloc[pending["trigger_high_idx"]]["high"])
            trigger = prior_high + krx_tick(prior_high)
            nxt_open = float(bar_next["open"])
            nxt_high = float(bar_next["high"])
            fill = None
            if nxt_open >= trigger:
                fill = nxt_open * (1 + slippage_rate)           # 갭상승 체결
            elif nxt_high >= trigger:
                fill = trigger * (1 + slippage_rate)            # 장중 스톱 발동
            if fill is not None:
                ...
                if qty > 0:
                    ...
                    trades.append({
                        ...
                        "datetime": str(bar_next["datetime"]), "price": fill,
                        "qty": qty, "reason": ",".join(pending.get("reasons") or ["signal"]),
                        "entry_price": fill, "pnl_pct": 0.0,
                    })
                pending = None
            else:
                # 미체결 → 다음날 재시도. trigger는 가장 최근 봉(i+1) 고가로 갱신.
                pending["days_left"] -= 1
                pending["trigger_high_idx"] = i + 1
                window_close = df["close"].iloc[: i + 2].astype(float)
                if pending["days_left"] <= 0 or not screen1_uptrend(window_close):
                    pending = None  # N_TRAIL 초과 또는 추세 반전 → 신호 취소

        # 신호 평가 (무포지션 & 대기 신호 없을 때)
        if position is None and pending is None:
            ...
            if signal is not None and signal.signal_type in (SignalType.BUY, SignalType.STRONG_BUY):
                # Screen 3 매수스톱 대기 등록 (trigger = high[i] + 1틱). 다음 봉부터 판정.
                pending = {
                    "trigger_high_idx": i,
                    "days_left": N_TRAIL,
...
def main():
    ...
    p.add_argument("--reports-dir", default="reports/books_research/elder_triple_screen")
    ...
    for mode, rule_name in combos:
        ...
        for code, df in data.items():
            res = simulate_one_stock(
                ...
                trail_ema=params["trail_ema"],
                trend_flip_exit=params["trend_flip_exit"],
            )
