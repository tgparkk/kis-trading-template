"""Minervini VCP — BULL/BEAR/SIDEWAYS 국면 분해.

KS11(코스피) 일봉이 daily_prices에 없으면 universe 중앙값 수익률로 대체.

출력: reports/books_research/minervini_vcp/regime_breakdown.parquet
...
LOG = logging.getLogger("regime_split_minervini")
logging.basicConfig(level="INFO", format="%(asctime)s %(levelname)s %(name)s: %(message)s")

OUT_DIR = ROOT / "reports" / "books_research" / "minervini_vcp"
...
def _try_load_ks11(start: str, end: str) -> pd.Series | None:
    """daily_prices에서 KS11 또는 유사 코드로 지수 종가 로드 시도."""
    from db.connection import DatabaseConnection
    candidates = ["KS11", "^KS11", "0001", "KOSPI"]
    with DatabaseConnection.get_connection() as conn:
        cur = conn.cursor()
    ...
    for i, dt in enumerate(dates):
        ...
        if ks11 is not None and dt in ks11.index:
            ...
            if past_dt in ks11.index:
                ...
                # KS11 과거 데이터 없으면 universe 중앙값 20일 누적
                sub = wide_close.iloc[i - 20: i + 1]
                med_ret_20d = float((sub.iloc[-1] / sub.iloc[0] - 1).median())
                ret_20d = med_ret_20d
                note = "universe_median_fallback"
        else:
            # KS11 없음 — universe 중앙값 20일 누적 수익률
            sub = wide_close.iloc[i - 20: i + 1]
            med_ret_20d = float((sub.iloc[-1] / sub.iloc[0] - 1).median())
            ret_20d = med_ret_20d
            note = "universe_median_fallback"
...
def main():
    ...
    if ks11 is None:
        LOG.warning("KS11 not found in daily_prices — using universe median return as proxy")
