"""v3 summary + changelog 최종 버전 작성 (corrected metrics)"""
import os

P5_DIR = r"D:\GIT\kis-trading-template\RoboTrader_template\reports\10pct_strategy\phase5_signals"
CL_DIR = r"C:\Users\sttgp\.claude\projects\D--GIT-kis-trading-template\memory"

# ── v3_summary.md ──────────────────────────────────────────────────────────
summary = """# Phase 5 v3 — OBV+VWAP Swing Portfolio Walk-Forward (1-page)
...
with open(os.path.join(P5_DIR, "v3_summary.md"), "w", encoding="utf-8") as f:
    f.write(summary)
print("v3_summary.md OK")

# ── changelog ─────────────────────────────────────────────────────────────
changelog = """# Phase 5 v3 Portfolio Walk-Forward 최종 결과 (2026-05-26)

## 작업 요약
직원 #9 (Phase 5 최종 라운드 v3 재설계). architect 진단 (b) 구현.
직원 #8 절단 재시도. 직원 #4/#5/#6/#7 실패 패턴 회피.

## v3 재설계 내용
- Universe: ROE Q4+ 교집합 제거 → mcap top 500 + tv 5d median > 10억
- Signal: OBV(lb=5, slope>=1.0sigma), VWAP pullback(pb_10), OBV_OR_VWAP
- Exit: SL[-5,-7,-10%], TP[3,5,10%], TM[1,3,5]d
- Portfolio: 1,000만원, 동시 5포지션, equal weight, T+1 시가, 0.3% 편도
- Walk-forward: 252/63 x 16 windows (단독 검증 동일)

## 최종 결과 (정직 보고)
- 합격 셀: 0 / 81 (전 family 게이트 미통과)
- WF window 월평균: -4.37% (1차 +0.23% 대비 -4.60%p 악화)
- 양수 WF 윈도우: 2/16 (12.5%)
- Sharpe(연환산): -3.64
- MDD(WF window 누적): -48.25%
- 목표 진척률: -43.7% (역방향)

## Top cell (family당 best_mean)
- OBV: mean_pnl=-0.248% sharpe=-0.56 n=55,094
- VWAP: mean_pnl=+0.590% sharpe=1.36 n=39,021 (유일 양수)
- OBV_OR_VWAP: mean_pnl=+0.054% sharpe=0.12 n=86,798

## Cross-section vs Trade-level
- 단독 OBV cross-section: +172.36bps (5/25 16/16 양수)
- v3 OBV trade-level: -24.8bps (회수율 -14.4%)
- 신호 방향 맞으나 SL intraday 빈번 발동 → alpha 소멸

## 판정
최종 종결 — 현재 알파(OBV+VWAP)로 월 10% 도달 불가
...
## 다음 단계 (사장님 결재 대기)
- P1: VWAP 단독 paper 5영업일 (trade-level +0.59% 유일 양수)
- P2: OBV exit 재설계 (ATR-based adaptive SL)
- P3: 분봉 2023~2024 백필 (VWAP IS 샘플 확보)
- P4: OBV_OR_VWAP 폐기
- P5: 외부 시그널 탐색 (외국인순매수, 공매도비율)

## 산출물
- RoboTrader_template/scripts/10pct_strategy/p5_obv_swing_walkforward.py (신설)
- reports/10pct_strategy/phase5_signals/v3_grid_all.csv (81 cells)
- reports/10pct_strategy/phase5_signals/v3_walkforward.csv (16 windows)
- reports/10pct_strategy/phase5_signals/v3_monthly_pnl.csv (47 months)
- reports/10pct_strategy/phase5_signals/v3_summary.md
"""

os.makedirs(CL_DIR, exist_ok=True)
cl_path = os.path.join(CL_DIR, "changelog-2026-05-26-v3-portfolio.md")
with open(cl_path, "w", encoding="utf-8") as f:
    f.write(changelog)
print(f"changelog OK: {cl_path}")
