...
def _scenario_rows(merged: pd.DataFrame, index_df: pd.DataFrame,
                   feat: str = "amihud", hold_n=None) -> List[dict]:  # pragma: no cover
    rows = []
    for h in HORIZONS:
        label = f"fwd_{h}d"
        ...
        for tp in TOP_PCTS:
            # gross(무비용) 참조
            g = build_periods(merged, feat, label, tp, h, 0.0, 0.0, 0.0, hold_n=hold_n)
            gross_stats = period_stats(g["net"], ppy)
            for sc, (ft, sl, sh) in COST_SCENARIOS.items():
                per = build_periods(merged, feat, label, tp, h, ft, sl, sh, hold_n=hold_n)
...
def build_report(merged: pd.DataFrame, index_df: pd.DataFrame, hold_n=None) -> str:  # pragma: no cover
    tbl = pd.DataFrame(_scenario_rows(merged, index_df, hold_n=hold_n))
...
def capacity_rows(merged: pd.DataFrame, hold_n=None) -> List[dict]:  # pragma: no cover
    ft, sl, sh = HEAD_COST
    ...
    for coef_name, coef in IMPACT_COEFS.items():
        for cap in CAPITALS:
            per = build_periods(merged, "amihud", label, HEAD_TP, HEAD_H, ft, sl, sh,
                                capital=cap, impact_coef=coef, hold_n=hold_n)
            st = period_stats(per["net"], ppy)
            rows.append({
...
def yearly_alpha_rows(merged: pd.DataFrame, index_df: pd.DataFrame, hold_n=None) -> List[dict]:  # pragma: no cover
    ft, sl, sh = HEAD_COST
    label = f"fwd_{HEAD_H}d"
    ppy = TRADING_DAYS_YR / HEAD_H
    per = build_periods(merged, "amihud", label, HEAD_TP, HEAD_H, ft, sl, sh, hold_n=hold_n)
...
def build_reduced_n_report(merged: pd.DataFrame, index_df: pd.DataFrame,
                           hold_n: int) -> str:  # pragma: no cover
    """축소 N 에서 전체 멀티버스 재측정(비용×호라이즌×분위 + 알파 + 연도별 + 수용량).

    N 축소는 종목당 자본↑ → 참여율↑ → capacity 천장↓ 라 수용량 재측정이 특히 중요.
    """
    body = build_report(merged, index_df, hold_n=hold_n)
    body = body.replace("# amihud Cost-Aware 분위 포트폴리오 백테스트 (Phase 0+ — 측정 전용)",
                        f"# amihud N={hold_n} 멀티버스 재측정 (분산축소 검증 — 측정 전용)")

    yr = pd.DataFrame(yearly_alpha_rows(merged, index_df, hold_n=hold_n))
    yr_show = yr.assign(net_ann=(yr["net_ann"] * 100).round(2),
                        mkt_ann=(yr["mkt_ann"] * 100).round(2),
                        alpha_ann=(yr["alpha_ann"] * 100).round(2),
                        beta=yr["beta"].round(2),
    ...
    cap = pd.DataFrame(capacity_rows(merged, hold_n=hold_n))
    cap_show = cap.assign(
        capital=(cap["capital_krw"] / 1e8).round(1).astype(str) + "억",
        mean_impact=(cap["mean_impact"] * 100).round(2),
        net_cagr=(cap["net_cagr"] * 100).round(2), net_sharpe=cap["net_sharpe"].round(2),
    ...
    extra = [
        "", f"## 연도별 알파 @N={hold_n} (net base 비용, KOSPI 대비)",
        "축소 N 에서도 2022 약세장 알파가 유지되는지.", "",
        yr_show.to_markdown(index=False), "",
        f"## 수용량 @N={hold_n} — 제곱근 시장충격 가산 후 net",
        f"★N 축소 핵심 점검: 종목당 자본 = 총자본/{hold_n} 로 N=83 대비 커져 참여율↑·천장↓.", "",
        cap_show.to_markdown(index=False), "",
        "## 판독",
        f"- N={hold_n} 헤드라인(h=20·base) net_sharpe·alpha 가 N=83 대비 유지되면 축소 타당.",
...
def main():  # pragma: no cover
    ...
    ap.add_argument("--reduced-n", type=int, default=15, help="축소 N 멀티버스 재측정")
    args = ap.parse_args()
    ...
    out3 = os.path.join(rdir, f"amihud_n{args.reduced_n}_multiverse.md")
    with open(out3, "w", encoding="utf-8") as f:
        f.write(build_reduced_n_report(merged, index_df, args.reduced_n))
    print(f"[amihud-bt] {out} / {out2} / {out3} (merged rows {len(merged)})")
