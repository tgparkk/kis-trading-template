"""강창권 『주식투자 단기 트레이딩의 정석』 — 일봉(daily) 백테스트.

분봉 백테스트(run_haru_silijeon_minute.py)와 별개. A등급 일봉 7종을 daily_prices 로 검증.

usage:
  python scripts/run_haru_silijeon_daily.py --variant A --all-modes
  python scripts/run_haru_silijeon_daily.py --variant A --mode single --rule daily_ma20_pullback
  python scripts/run_haru_silijeon_daily.py --variant B --mode single --rule daily_new_high_breakout
  # 파일럿(짧은 기간·단일룰):
  python scripts/run_haru_silijeon_daily.py --variant A --mode single \
      --rule daily_ma20_pullback --start 2021-01-01 --end 2026-05-29 --limit 50

데이터: daily_prices (OHLC adj_factor 적용 수정주가)
universe: top_volume:50 (일평균 거래대금 상위 50)  — 기존 일봉책(elder)과 동일
청산(공통 원칙 "이평선 이탈 시 매도" + A-07만 +10% 익절 명시):
  Variant A (스윙 표준): sl 8% / tp off(99%) / mh 100 / trail_ma=룰별 기본
  Variant B (단기 회전): sl 8% / tp 12% / mh 20 / trail_ma=None
  ※ A-07(daily_ma20_pullback)은 책 명시 +10% 익절 → variant 무관 tp=0.10 강제 override.
no-lookahead: rule 은 df[:i+1]만, 체결은 다음 봉 시가. 거래비용 elder/문병로와 동일.
...
from strategies.books.haru_silijeon.strategy_daily import BOOK_META_DAILY, build_strategy_daily

LOG = logging.getLogger("haru_silijeon_daily")
...
# A-07 책 명시 익절 (+10%) — variant 무관 강제 override
RULE_TP_OVERRIDE = {
    "daily_ma20_pullback": 0.10,
}

# 룰별 기본 trail MA(일봉 이평선 이탈 시 매도). variant A use_trail=True 일 때만 적용.
# all_AND 모드는 진입 이평이 혼재되므로 보수적으로 20일선 사용.
RULE_TRAIL_MA = {
    "daily_ma20_pullback": 20,
    "daily_ma5_10_follow": 10,
    "daily_ma60_doji_rebound": 60,
    "daily_trend_filter_240_480": 240,
    "daily_swing_pullback": 20,
    "daily_new_high_breakout": 20,
    "daily_vol300_longma_break": 240,
}
DEFAULT_TRAIL_MA = 20
...
def _resolve_exit_params(variant: str, mode: str, rule_name: Optional[str]):
    ...
    # A-07 +10% 익절 override (single 모드에서 해당 룰일 때)
    if mode == "single" and rule_name in RULE_TP_OVERRIDE:
        tp = RULE_TP_OVERRIDE[rule_name]
    # trail MA
    trail = None
    if params["use_trail"]:
...
def main():
    ...
    p.add_argument("--reports-dir", default="reports/books_research/haru_silijeon_daily")
