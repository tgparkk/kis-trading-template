...
🟢🔒🆕 **M4 채널은 확정됐고, 정의 재해석은 v0.6(2026-09-15 오후 사장님 승인)으로 발효됐다** —
`TRACE_M4_channel_2026-09-15.md`: D 15:3x 정규장 INSERT → D+1 09:00 라이브 판독 → D+1 15:3x 의
7봉 UPSERT 가 **시간외 단일가를 사후 합산**한다. 룰 충실도는 「아직 재기록되지 않은 하루」
자연실험(09-14)이 **20/20 `ratio = 1.00000000`** 으로 따로 보였다. ~~🔴 정의 재해석은 🔒 사전등록
개정 사안이고~~ **문턱은 그대로 불변**이다. `--vintage-adjust` 는 그 재해석을 **인쇄로만** 보이는
보조 판이지 판정이 아니다(설계서 §4-3 v0.6 정본).

**요지 5줄(설계서 §4-3 v0.6 정본 · `GATE_REPORT_2026-09-15.md` 근거):**

1. **M4 = 빈티지 동일성 지표.** 정의·문턱(상대오차 ≤1e-6 행 비율 ≥99%)은 불변 — 재는 것은 룰
   충실도가 아니라 「재현기 입력이 라이브 09:00 판독 시점과 같은 빈티지인가」다.
2. **룰 충실도 판정 = M1·M2·M3 뿐.** M4 는 「빈티지 동일 / 빈티지 불일치」 한 줄만 별도 기재하고,
   **M4 미달 단독으로 FAIL 을 선언하지도, 조건부 통과 사유로 쓰지도 않는다.**
3. 빈티지 불일치 하에서 M1~M3 가 통과하면 판정문에 「빈티지 불일치 하 통과」를 **의무 병기**하고
   `--vintage-adjust` 판 M1~M4 를 나란히 인쇄한다.
4. 🔴 **사후 완화 고지** — 이 개정은 게이트 결과(M4 0.05~0.18)를 본 «뒤» 이루어졌다. 개정 시점
   판정은 여전히 FAIL/조건부(daytrading M1 0.9429 < 0.95 중단선)이고, **통과로 바뀐 세그먼트는
   0개**다(설계서 §4-3 표).
5. M1·M3 도 빈티지에 미세 민감(보정 판 daytrading M1 0.9424→0.9493) — 다음 게이트 재판정
   (**2026-09-30 이후** · 보호 구간 ≥15일) 때 보정 판을 **반드시 병기**하되 **판정은 원판**.

🔴 **lag 표는 판별 장치가 아니다** — `implied_k = stored_k − Δ`, `Δ = 20·(replay − live)` 가
**k 에 무관**이라 `ratio_k = 1 − Δ/stored_k` 는 분모 크기 차이만 반영한다. 평탄해도 한 봉 채널의
반증이 아니고, `k=0` 이 전부 `< 1` 인 것도 증거가 아니다(기록용).
