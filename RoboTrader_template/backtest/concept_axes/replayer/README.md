...
🟢🆕 **M4 채널은 확정됐다 — `TRACE_M4_channel_2026-09-15.md`**: D 15:3x 정규장 INSERT →
D+1 09:00 라이브 판독 → D+1 15:3x 의 7봉 UPSERT 가 **시간외 단일가를 사후 합산**한다.
⇒ ***현행 M4 는 룰 충실도 지표가 아니라 「데이터 빈티지 동일성」 지표다*** — 룰 충실도는
「아직 재기록되지 않은 하루」 자연실험이 **20/20 `ratio = 1.00000000`** 으로 따로 보였다.
🔴 **정의 재해석은 🔒 사전등록 개정 사안이고 문턱은 불변이다.** `--vintage-adjust` 는
그 재해석을 **인쇄로만** 보이는 보조 판이지 판정이 아니다.
