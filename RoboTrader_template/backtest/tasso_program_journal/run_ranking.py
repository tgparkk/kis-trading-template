...
def post6_main(a):
    ...
    say("🔴🔴 **그러나 「줄바꿈 잣대 차이일 뿐」이라고 «결론지을 수 없다».** "
        ...
        "말하지 않는다.**")
    say("")
    say("🟢 **post6 «판정 경로»의 불변은 md5 가 아니라 «재실행»으로 증명한다** — "
        "고정 창(`python run_ranking.py --stage post6 --upto 2026-09-04`)으로 다시 돌려 "
        "`RESULTS_RANKING_POST6*.md` 가 **0 diff** 로 재생되는지를 본다. "
        "🔑 ***이것이 「같은 입력 → 같은 출력」의 «유일한» 증거이고, "
        "md5 일치는 그 증거의 대용이 될 수 없다.***")
    say("")
    ...
    say("🔑 **계열 규칙 후보**: ***md5 를 동결할 때는 ① 「작업트리 md5」인지 「블롭 md5」인지, "
        ...
        "**동결 시점 스냅샷의 이름표**일 뿐이고, 불변 주장은 «고정 창 재실행 0 diff»로 해야 한다.")
    say("")
