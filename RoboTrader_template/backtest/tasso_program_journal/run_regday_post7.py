...
def main():  # noqa: C901
    ...
    if blocked:
        ...
        # 🔴 `v_main`(= "✅ 지지")을 «낱말째» 인쇄하면 그 줄만 떼어 인용된다(A-N2).
        #    ⇒ 기호도 「지지」라는 낱말도 쓰지 않고 **「충족」**만 적는다.
        say("\n⇒ 🔒 **결정규칙 «비율 ≥ 0.8333 ∧ 귀무 백분위 < 5%» 는 두 성분 모두 "
            "«충족»됐다 — 🔴 «기록»이고 «선언»이 아니다.**")
        say("🔴 **이 항목에 ✅ 나 「지지」라는 낱말을 붙이지 않는다** — 아래 의존 단서를 «먼저» 읽는다.")
        say("⇒ ⛔ 판정 불가 조건 ①~④ **전부 미발동**"
