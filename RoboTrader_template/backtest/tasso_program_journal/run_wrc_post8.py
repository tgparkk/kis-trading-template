...
사전등록·동결(값을 보기 «전»에 고정 · 1순위 출처):
  ...
  · `PREREG_POST8.md`(동결 `729f28b`) §3 `D-3` · §4 `D-4` · §5 `D-5` · §8 `D-8` · §9 `D-9` · §0-5-1 가분성
...
🔴 **읽은 시각(D-9 ①)과 바이트 결정론의 절충(재량 · 보고서에 신고)** — `PREREG_POST8.md` §9 (나) 2 는 쿼리 실행
   ...
   **이번 실행의 벽시계 시각은 stdout** 으로 낸다. 지문이 바뀌면(= 다른 빈티지) 새 시각이 박힌다.
...
def main() -> int:  # noqa: C901, PLR0912, PLR0915
    ...
    say("")
    say("⇒ 🔴 **6항목 전부 «지지»도 «불성립»도 아니다.** 🔑 ***「못 좁힌다」도 「잴 건이 없었다」도 아니라 「못 쟀다」다***"
    ...
    for d, st, nb in cod_res:
        ...
        n_i = n_cum + 1
        r_i = (sub_a + sub_z) / n_i
        say(f"| `approx` 포함(코데즈컴바인 D={d} · 창 `[{d}, {DB_UPTO}]` {nb}봉 · `A0` {st}) | "
            f"민감도(🔴 `D-3` 우선) | **{n_i}** | 충족(🔴 `approx` 로 채운 n — 판정 분모 아님) | "
            f"{sub_a + sub_z}/{n_i} = {100 * r_i:.1f}% | "
            f"**{'⛔ `WRC-G1` 발동' if r_i >= G1_THR else 'G1 미발동(나머지 미계산 ⇒ 미룸)'}** | "
