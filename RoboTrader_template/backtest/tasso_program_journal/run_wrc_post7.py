...
def main() -> int:
    ...
    say("| 항목 | 가설(한 줄) | 문턱 | 이번 분모 | **판정** | 대칭/반증 쌍 | ⛔ 경로 |")
    say("|---|---|---|---|---|---|---|")
    rows = [
        ("`WRC-P1`", "가중 평단 복원의 `b_k` 밴드 폭이 3%p 미만",
         f"{BAND_THR}%p (`FREEZE_WRC` §3)", "`WRC-N2`",
         "판정 분모 0 < 3 ⇒ **잴 건이 없다**"),
        ("`WRC-N1`", "(대칭) 판별력 — 대조군이 관측만큼 맞히면 무정보",
         f"{N_THR:.0%} (`RESULTS_D1_OOS_POST5.md` §9 W7 차용)", "`WRC-P1`",
         "분모 0 ⇒ 대조군을 만들 표본이 없다"),
        ("`WRC-N2`", "(대칭) 밴드가 3%p 이상이면 `P1` 철회",
         f"{BAND_THR}%p", "`WRC-P1`", "분모 0"),
        ("`WRC-G1`", "커버리지 손실 — 게이트 탈락 비율",
         f"{G1_THR:.4f} (1/3)", "자신이 반증축",
         "🔴 **0/6 = 「측정 가능 0」** ⇒ 문턱 대조 이전에 **최소 n 미달**"),
        ("`WRC-O1`", "훈련(탐색 post4·5) ↔ 검정(post6·7) 병기",
         "(병기 의무 · 문턱 없음)", "자신이 반증축",
         "post7 열이 **비어 있다** ⇒ 병기만 하고 비교하지 않는다"),
        ("`WRC-V1`", "(대칭) 갈래를 바꾸면 답이 갈리나",
         "갈리면 **선언 금지**", "자신이 반증축",
         "🔴 **갈래를 만들 수 없다** — `exact` 0 · `approx` 1 둘 다 3 미만"),
    ]
    for lbl, hyp, thr, pair, blk in rows:
        say(f"| {lbl} | {hyp} | {thr} | **{n_main}** | ⛔ **판정 불가(최소 n 미달)** | {pair} | {blk} |")
    say("")
    ...
    say("- 🔴 이번 글은 **분모가 0** 이라 `G1` 의 «비율»을 만들 수 없다. "
