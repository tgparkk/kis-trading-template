# -*- coding: utf-8 -*-
"""`WRC-` 축 **판정** 실행 — 7번째 글 · 🔴🔴 **판정 분모 = «누적» 5**(`WRC-R11` · post6 5 + post7 0 ≥ 3 ⇒ 게이트 열림)
⇒ **⛔ 사유 = `WRC-G1` 4/5 = 80.0% 발동**. 🔴 post7 «단독» 분모 0 은 **병기**할 뿐 「최소 n 미달」의 근거가 아니다.
...
🔴🔴 **post7 «단독» 주 분모가 구조적으로 «0» 이다.** `WRC` 판정 조건(`exact` ∧ `fill_n >= 2` ∧
   서로 «다른» 값 레그 >= 3)에 대해 `exact` **6**건 중 **5건이 1차 체결**(`fill_n = 1`)이고,
   남은 해치텍(`fill_n = 4`)은 **서로 다른 값 레그가 2 < 3** 이다.
   🔑 ***값을 보고 닫은 것이 아니라 «구성»으로 닫혔다*** — 저자의 체결 차수 분포(§2-12: 1차 **6건**)가
   닫은 것이고, 우리가 고른 것이 아니다.

🔴🔴 **그러나 그것이 «판정 분모»는 아니다.** §6-1 `WRC-R11`(`PREREG_WEIGHTED_RECON.md:748-753`)의
   «누적 규약»이 *「누적 3건이 되는 즉시 한 번 판정한다」* + *「이후 글에서는 **누적 전체로
   다시 계산**한다(같은 검정의 갱신)」*를 «둘 다» 못박는다 ⇒ **판정 분모 = post6 5 + post7 0
   = 5 ≥ 3** 이라 **게이트는 열린다**. 열린 뒤 «먼저» 닫는 것이 `WRC-G1` 이다
   (§4-6 · §7-B #24 — 가장 먼저 계산) ⇒ **⛔ 사유 = 누적 `WRC-G1` 4/5 = 80.0% 발동**.
   🔑 ***「건수가 늘면 열린다」가 아니라 「커버리지가 좋아져야 열린다」*** — 두 사유는
   다음 글에서 열리는 «조건»이 다르다.
...
COVERAGE = [
    ...
    ("`S5`", "`dart_financials_asfiled` PIT", "5/6", "1(해치텍 — 재무 표 미수록)", "16.7%",
     "(판정 안 함 · PD-15)"),
    # 🔴 「측정 가능」 칸은 **post7 단독**(0/6)이고, ⛔ 사유는 **누적** `WRC-G1` 이다(§3).
    ("**`WRC-`(`WRC-G1`)**", "`fill_n >= 2` ∧ 서로 다른 값 레그 >= 3",
     "**0/6**(post7 단독)", "—", "— (누적 4/5 = **80.0%**)",
     "🔴 **누적 `WRC-G1` 80.0% 발동으로 ⛔**(최소 n 미달이 «아니다» · §3)"),
]
...
def main() -> int:
    ...
    say(f"- 🔴🔴 **post7 «단독» 주 분모 = {n_main}건.** "
        "🔴 **그러나 이것으로 「최소 n 미달」을 선언하지 않는다** — §6-1 `WRC-R11` 의 "
        "«누적 규약»대로 **판정 분모는 누적**"
        f"(post6 {P6_WRC['denom']} + post7 {n_main} = "
        f"**{P6_WRC['denom'] + n_main}** ≥ 3 ⇒ 게이트 열림)이고, "
        "⛔ 사유는 **누적 `WRC-G1` 80.0% 발동**이다(§3).")
