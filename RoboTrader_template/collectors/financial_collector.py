...
def _build_summary(d: date, fetcher, n_filings: int, n_accounts: int, n_kis_ok: int,
                    ...
                    targets_total: int, targets_processed: int, unmapped: int) -> dict:
...
def collect_financials(target_date: str = None, daily_cap: int = 800) -> dict:
    ...
    quota_hit = cap_hit = blocked = False
...
def reconcile_financials(trade_date: str) -> dict:
    ...
    if not reachable:
        ...
        logger.error(
            "[financials] reconcile: 도달성 실패(status∉{000,013} 또는 차단) - "
