...
_FRESH = {
    ...
    "005930": ["2026-09-10"], "000660": ["2026-09-10"], "035420": ["2026-09-10"],
}
...
def test_freshness_warning_when_index_lags(monkeypatch, caplog):
    ...
    assert set(res) == {"KOSPI", "KOSDAQ"}
    assert "[index-freshness] STALE axis=A table=daily_prices" in caplog.text
    assert "src=kis" in caplog.text


def test_no_freshness_warning_when_fresh(monkeypatch, caplog):
    import core.regime.index_refresh as ir
    _at(monkeypatch, ir)
    kis = _FakeKIS({"0001": _kis_df(["20260910"]), "1001": _kis_df(["20260910"])})
...
def test_premarket_0740_does_not_false_alarm(monkeypatch, caplog):
...
def test_row_count_log_keeps_prefix_and_adds_src(monkeypatch, caplog):
    ...
    assert "[regime-index] KOSPI(KS11) 1행 갱신 src=kis" in caplog.text
