...
# =============================================================================
# 지수 일봉 소스 — KIS 전환 + 신선도 경보 (2026-09-10)
#   spec: docs/superpowers/specs/2026-09-10-index-kis-freshness-design.md
# =============================================================================
# 2026-09-08~09: 상류(FDR KS11/KQ11)가 죽었는데 «옛 6봉»을 정상 반환해 index_daily·
# daily_prices 의사티커가 09-07 에서 2거래일 멈췄고 로그·EOD 요약은 전부 정상이었다.
# 「N행 갱신」은 「오늘 것이 들어왔다」의 증거가 아니다 ⇒ 소스를 KIS 로 옮기고,
# «행 수»가 아니라 «날짜»를 보는 신선도 판정을 붙인다.
#
# 🔴 롤백은 이 값을 "fdr" 로 되돌리는 것 «하나»뿐이다. 단 롤백이 이미 쓰인 데이터를
#    되돌리지는 않는다. 그리고 신선도 검사는 이 스위치 «밖»이라 "fdr" 로 내려도 계속
#    돈다 — 이 결함을 잡았어야 할 장치를 롤백이 같이 꺼버리면 안 된다(설계 §3·§8).
INDEX_DAILY_SOURCES = ("kis", "fdr")
INDEX_DAILY_SOURCE = "kis"

# 신선도 «기준일 오라클» — 달력·휴장일 문제를 달력이 아니라 데이터로 푼다.
