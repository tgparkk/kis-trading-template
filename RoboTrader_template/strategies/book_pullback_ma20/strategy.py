"""
Book Pullback MA20 Strategy — 강창권 『단기 트레이딩의 정석』 A-07 실전판
=======================================================================

강창권 『단기 트레이딩의 정석』 A-07, 백테스트 Sharpe 0.44/+16% — 급등 후
20일선 눌림 지지 양봉.

백테스트 검증판을 실전 파이프라인(BaseStrategy / on_tick)으로 코드화한 전략.

진입 신호는 백테스트와 1:1 일치를 보장하기 위해
``strategies/books/haru_silijeon/rules_daily.py`` 의 헬퍼를 직접 재사용한다:
  - ``rule_daily_ma20_pullback`` : 급등 이력 + 20일선 눌림 지지 양봉
  - ``_ma`` : trailing SMA 계산 (청산 trailing 기준선)

진입 (rule=daily_ma20_pullback):
  1. 직전 30일 내 +25% 급등 이력
...
청산 (A-07 variant):
  - 손절: -8%
  - 익절: +10% (A-07 책 명시 유일 익절)
  - 최대 보유: 50거래일 (주말/공휴일 제외 — korean_holidays)
  - MA20 trailing: 수익 중(ret>0) 종가가 20일선 하향 이탈 시 청산
  (trend_flip 없음)
...
class BookPullbackMa20Strategy(BaseStrategy):
    """강창권 A-07 — 급등 후 20일선 눌림 지지 양봉 진입 (스윙)."""

    name: str = "BookPullbackMa20Strategy"
    version: str = "1.0.0"
    ...
    def on_init(self, broker, data_provider, executor) -> bool:
        ...
        self._max_hold_days = int(risk.get("max_hold_days", 50))
        ...
        self.logger.info(
            ...
            f"(MA20 눌림, sl={self._stop_loss_pct:.0%}/tp={self._take_profit_pct:.0%}/"
    ...
    def evaluate_entry(
        df: pd.DataFrame,
        min_daily_bars: int = 35,
    ) -> Tuple[bool, List[str], Dict[str, Any]]:
        """진입 조건 평가 — 백테스트 rule_daily_ma20_pullback을 그대로 호출.
