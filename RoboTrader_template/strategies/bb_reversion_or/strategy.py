"""
BB Mean Reversion Strategy — OR-relaxed variant (bb_reversion_or)
==================================================================

Derived from bb_reversion. Entry condition relaxed from 4-AND to:
  BB ∧ (RSI ∨ VOL) ∧ ADX

Buy conditions:
  1. Price <= BB lower band (20-day, 2 sigma)          [required]
  2. RSI(14) < oversold  OR  Volume >= avg * ratio     [at least one]
  3. ADX(14) < adx_max (sideways confirmation)         [required]
...
Differs from bb_reversion only in evaluate_buy_conditions.
Do NOT modify bb_reversion/ — this is an isolated experiment variant.
"""
...
class BBReversionORStrategy(BaseStrategy):
    """BB mean reversion — OR-relaxed entry: BB ∧ (RSI ∨ VOL) ∧ ADX."""

    name: str = "BBReversionORStrategy"
    version: str = "1.0.0-or"
    description: str = "BB lower band buy, BB middle sell — OR-relaxed: BB ∧ (RSI ∨ VOL) ∧ ADX"
    author: str = "Template"
    ...
    def evaluate_buy_conditions(
        ...
        # 2. RSI OR VOL  [at least one required — OR-relaxed]
        rsi_ok = rsi_value < rsi_oversold
        vol_ok = volume_ratio >= volume_ratio_min
        if not (rsi_ok or vol_ok):
            return None
        if rsi_ok:
