"""
Book Pullback MA5 Strategy — 『트레이딩의 전설』(Book15) ma5_pullback 실전판
==========================================================================

트레이딩의 전설(Book15) ma5_pullback, 백테스트 Sharpe 0.63 — 단 BULL 청산
의존 리스크. 급등 후 5일선 눌림 지지 양봉.

백테스트 검증판을 실전 파이프라인(BaseStrategy / on_tick)으로 코드화한 전략.

진입 신호는 백테스트와 1:1 일치를 보장하기 위해
``strategies/books/trading_legends/rules_daily.py`` 의 헬퍼를 직접 재사용한다:
  - ``rule_ma5_pullback`` : 급등 이력 + 5일선 눌림 지지 양봉
  - ``_ma`` : trailing SMA 계산 (청산 trailing 기준선)

진입 (rule=ma5_pullback):
  1. 최근 20일 내 +20% 급등 이력
  2. 마지막 봉 저가가 5일선 부근 터치 (±touch_tol)
  3. 종가가 5일선 위 (지지 유효: close >= ma5 * (1 - below_tol))
  4. 마지막 봉 양봉 (지지 반등)

청산:
  - 손절: -3% (타이트 — 상따/단기 트레이더 손절)
  - 익절: +15%
  - 최대 보유: 30거래일 (주말/공휴일 제외 — korean_holidays)
  - MA5 trailing: 수익 중(ret>0) 종가가 5일선 하향 이탈 시 청산
  (trend_flip 없음)
...
class BookPullbackMa5Strategy(BaseStrategy):
    """트레이딩의 전설 ma5_pullback — 급등 후 5일선 눌림 지지 양봉 진입 (스윙)."""

    name: str = "BookPullbackMa5Strategy"
    version: str = "1.0.0"
    ...
    def get_min_data_length(self) -> int:
        """진입 룰(rules_daily.py)이 요구하는 최소 일봉 수."""
    ...
    def on_init(self, broker, data_provider, executor) -> bool:
        ...
        self._stop_loss_pct = float(risk.get("stop_loss_pct", 0.03))
        self._max_hold_days = int(risk.get("max_hold_days", 30))
        self._trail_ma = risk.get("trail_ma", 5)
        ...
        self.logger.info(
            ...
            f"(MA5 눌림, sl={self._stop_loss_pct:.0%}/tp={self._take_profit_pct:.0%}/"
    ...
    def generate_signal(
        ...
        # timeframe='intraday'로 분봉을 전달한다. trail_ma 청산을 분봉 MA5로
    ...
    def evaluate_entry(
        ...
        """진입 조건 평가 — 백테스트 rule_ma5_pullback을 그대로 호출.
    ...
    def evaluate_sell_conditions(
        ...
        # 4) MA trailing (수익 중에만)
        if trail_ma is not None and ret > 0:
            ma_trail = _ma(df, trail_ma)
