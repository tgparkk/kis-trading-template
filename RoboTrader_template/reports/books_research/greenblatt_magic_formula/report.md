...
> 펀더멘털 일봉을 strategy_analysis.daily_candles로 2021~2026 백필(+142,467행, market_cap=yearly_fundamentals 연도매칭) 후 재실행. n_eligible dates 124→**1241**.

| 룰 | 6개월(이전) | **5년(재검증)** |
|---|---|---|
| magic_formula_top A | 38T +10.04% Sharpe 0.41 (per-trade 84% win) | 308T +18.26% **Sharpe 0.12** (per-trade 50.7% win, +7.26%/거래) |
| magic_formula_top B | 197T +7.79% Sharpe 0.36 | 1724T +8.43% **Sharpe 0.05** (per-trade 47.7%, +0.75%/거래) |
| threshold / high_roc / all_AND | 0~ | 소표본 음수 (절대임계값 여전히 부적합) |

**핵심**: 다년·다국면으로 늘리니 **Sharpe 0.41→0.12 붕괴**, per-trade 승률 84%→50.7%(동전). 6개월 숫자는 단일 BULL 거품이었음. magic_formula_top은 여전히 Greenblatt 1위이나 risk-adjusted 엣지는 미미 → **CANDIDATE 부적격 재확인**. 0가격 행(671건) 가드 처리, zero_price_exits=0.
