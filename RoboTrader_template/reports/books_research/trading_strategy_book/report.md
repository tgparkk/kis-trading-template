...
**해석(잠정)**: 위 표는 **유니버스 top_volume:50(대형주)에 실행층만 단독 적용**한 결과다. **단, 이는 책 시스템을 충실히 재현하지 못한 불공정 측정**임이 §5.6에서 드러난다(책은 1층 급등주 후보선별 → 2층 장중실행의 2층 시스템). **§5.6 체이닝 적용 시 위 파국적 음수의 80~99%가 소멸** → 위 §5.2/§5.3 절대수치는 **유니버스 불일치 artifact**로 재해석해야 한다. **가상매매 미등록·라이브 무영향**(신규/수정 책 파일만, 기존 5전략·드라이버 무수정).
...
### 5.6 ★1층→2층 체이닝 재측정 — 파국적 손실은 유니버스 불일치 artifact였음 (2026-06-04)

> 계기: "책에 나온 내용인데 어떻게 전부 −98%인가, 빠뜨린 게 있지 않나"(사장님). → **있었다.** §5.2는 책의 **2층 시스템 중 2층(장중 실행)만 분리해 1층(매수후보 선별)을 건너뛴 채 대형주 풀(top_volume:50)에 적용**했다. 책 §0.1은 "거래량 급증+장대양봉 **급등주를 먼저 선별**한 뒤 그 종목에 장중 타이밍"을 명시한다.

**구현**: 드라이버에 `--candidate-screen`(1층 일봉 후보 게이트) 신규 추가 — 일봉 룰(`envelope_200d_high`=A~I) 통과 신호일 D → **D+1~D+3 거래일에만** 해당 종목 분봉 진입 허용(PIT, `--candidate-window 3`). `none`=무변경(바이트동일 검증). 신규 함수 `_eligible_dates_from_signals`/`_build_candidate_eligibility`/`_filter_cache_candidate` + 단위테스트 5(`tests/books/test_candidate_screen_gate.py`). PIT·no-op·배선 리뷰 APPROVE.

**후보 희소성**: A~I 통과 = period별 eligible 종목 6/8/5개, stock_days 62/72/39 (top_volume:50 중 극소).

| 룰 | 미체이닝 mPnl | **체이닝 mPnl** | 거래(미→체) | 체이닝 mSharpe | pos기간 |
|---|---|---|---|---|---|
| price_box (1분) | −98.5% | **−10.6%** | 8519 → 268 | −0.230 | 0/3 |
| bollinger (5분) | −75.4% | **−1.6%** | 2340 → 51 | +0.099 | 0/3 |
| pullback 4단계 (5분) | −21.9% | **−0.2%** | 474 → 4 | −0.081 | 1/3(OVERFIT) |

**느슨한 후보(책 §0.1)로 표본 확대 재측정**: A~I가 너무 희소(거래 4~268)해 엣지 판단이 어려워, **§0.1 "종목 선정 팁" = 거래량 평소 3배 급증 + 장대양봉(+3%)**을 신규 일봉 룰 `rule_volume_surge_candle`로 코드화(TDD 5)하고 동일 체이닝(window3) 재측정 → 표본 10배↑:

| 룰 | 미체이닝 | strict A~I | **loose 3배급증+양봉** |
|---|---|---|---|
| price_box (1분) | −98.5% (8519) | −10.6% (268) | **−40.4% (756)** |
| bollinger (5분) | −75.4% (2340) | −1.6% (51) | **−13.8% (141)** |
| pullback 4단계 (5분) | −21.9% (474) | −0.2% (4) | **−3.9% (42)** |

(loose 전부 양수기간 0/3.)

**★결론 정정(최종)**:
1. **파국적 음수(−75~−98%)의 대부분은 유니버스 불일치 artifact**였다 — 급등주 장중기법을 비후보(대형주)에 기계적으로 쏜 결과. 1층 후보로 거르니 손실 대폭 축소. **"책이 틀렸다"가 아니라 "측정이 불공정했다"**(사장님 지적이 옳았음).
2. **★단, 체이닝·표본확대 후에도 양(+) 엣지 입증 실패 — 오히려 강한 반증**: ①모든 유니버스·후보·전략·구간 조합에서 **양수기간 0/3**(strict pullback 1/3은 4거래 OVERFIT). ②**느슨한 후보로 표본을 늘리면 손실이 더 커짐**(bollinger −1.6%→−13.8%, pullback −0.2%→−3.9%, price_box −10.6%→−40.4%) = 추가된 거래가 net 손실 → 희소성 때문에 가려진 엣지가 아니라 **엣지 부재**가 드러남. **strict가 일관되게 덜 손실**(신고가 후보가 그나마 나음). → **판정: "파국적 부적격"은 artifact라 철회하되, "양(+) 엣지 부재"는 충분 표본으로 확인 → 실질 부적격(채택 불가).** 단 *파국적은 아님*(near-breakeven~중손실).
3. 잔여 한계(이 결론을 뒤집을 여지): ①청산이 드라이버 sl/tp/mh 근사 — **책 진짜 청산(장중 이등분선 트레일링·±3% 빠른 손익절)은 미모델**. 책의 엣지가 진입이 아니라 *타이트한 재량 청산*에 있다면 이 백테스트로는 영구히 측정 불가(분봉 백테스트 구조적 한계, 18·19권 공통). ②3개월·BEAR 부재. ③장중 재량(셋업 선별) 미모델.

**재현(체이닝)**: 위 §5.5 커맨드에 `--candidate-screen {envelope_200d_high|volume_surge_candle} --candidate-window 3` 추가. TSV `D:\tmp\multiverse\book19_{chain,loose}_{pricebox,bollinger,pullback}\`.
