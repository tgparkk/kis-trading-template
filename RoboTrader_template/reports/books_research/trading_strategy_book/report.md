...
**★결론 정정**:
1. **파국적 음수(−75~−98%)의 80~99%는 유니버스 불일치 artifact**였다 — 급등주 장중기법을 비후보(대형주)에 기계적으로 쏜 결과. 1층 후보로 거르니 손실이 거의 본전(−0.2~−10.6%)으로 붕괴. **"책이 틀렸다"가 아니라 "측정이 불공정했다".**
2. **단, 체이닝해도 양(+) 엣지는 입증 안 됨**: 전부 순손실(미미)·양수기간 0~1/3·거래 4~268로 희소 → **판정 = "파국적 부적격" 철회, "결론 보류(near-breakeven·표본부족·OVERFIT)"로 정정.** 채택도 기각도 아닌 *미결*.
3. 잔여 한계(여전히): ①청산이 드라이버 sl/tp/mh 근사(책 진짜 장중 이등분선 트레일링·±3% 빠른 청산 미모델) ②3개월·BEAR 부재 ③A~I는 엄격 스윙스크린이라 희소 — 책 §0.1 **느슨한 후보(거래량 3배 급증+양봉)**로 게이팅하면 표본↑ 가능(미실행, 후속). ④장중 재량(어떤 셋업을 고를지) 미모델.

**재현(체이닝)**: 위 §5.5 커맨드에 `--candidate-screen envelope_200d_high --candidate-window 3` 추가. TSV `D:\tmp\multiverse\book19_chain_{pricebox,bollinger,pullback}\`.
