> **DEPRECATED 2026-09-17 — 보관용.** 이유: 문서가 가리키는 `scripts/save_portfolio_snapshot.py`·`scripts/view_portfolio_snapshot.py`·테이블 `portfolio_snapshots`·`main.py` 자동 저장 로직·SQLite `data/robotrader.db` 가 현재 트리에 전부 없다. 코드의 `SystemMonitor._save_portfolio_snapshot()`(`bot/system_monitor.py`)은 「포트폴리오 스냅샷 저장 기능 미구현 (스킵)」 디버그 로그만 남기는 스텁이고, 저장소는 PostgreSQL(`db/connection.py` psycopg2 풀)이다.
> 대체 문서: 없음(기능 미구현). 전략별 자본 추적은 `docs/PAPER_STRATEGIES.md`(`paper_strategy_equity`).
> 원문은 아래 그대로.
