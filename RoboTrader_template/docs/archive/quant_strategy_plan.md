> **DEPRECATED 2026-09-17 — 보관용.** 이유: 2026-02-07 초기 커밋(`c2c38ef`)의 퀀트 전략 작업 계획. 「구현 현황」이 가리키는 `strategies/quant_strategy.py`·`strategies/combined_strategy.py`·`core/quant/quant_rebalancing_service.py`·`tests/test_quant_factors.py`·`tests/test_primary_filter.py`·`tests/test_rebalancing.py`·`backtests/quant_monthly_backtest.py` 는 현재 트리에 전부 없다(4개 — `quant_strategy.py`·`combined_strategy.py`·`quant_rebalancing_service.py`·`quant_monthly_backtest.py` — 는 git 이력에 한 번도 없었고(계획만), 테스트 3개는 초기 커밋 `c2c38ef` 에 있다가 `738ff1c`(2026-02-08 「quant/archived 전용 테스트 7개 삭제」)에서 삭제됨. 태그 `pre-quant-removal`(=`601d4f1` 2026-02-08)은 `main.py` quant 코드 제거(`30e14f8` 4단계) 직전 지점일 뿐, 그 시점 트리에도 이 7개 파일은 없다). 레포 안 인바운드 링크 0.
> 대체 문서: 현행 전략 허브 → `docs/PAPER_STRATEGIES.md`.
> 원문은 아래 그대로.
