...
- 🔑 **`volume`은 정반대 — 곱해야 맞는다** (2026-08-15 감사). close는 조정 저장인데 volume은 **원본 저장**이라 단위가 어긋나 있었다.
  실측 035720 2021-04-08: 그대로 곱한 거래대금 **1,004억** vs 실제 **5,020억**(인접 정상일 3,528억 대비 전자가 비현실적).
  깨지는 것 ①거래대금이 분할 이전 구간에서 `adj_factor`배 과소평가 ②**거래량 비율 룰이 분할 경계에서 왜곡** —
  `daytrading_3methods`의 「당일 ≥ 직전20봉 평균×2」가 분할 직후 20봉간 **가짜 매수신호**를 낸다(평균에 5배 작은 값이 섞임),
  `minervini` dry-up은 반대로 억제. 영향 규모: W-long 행의 2.44% · 종목 99개(3.8%) · 연 ~500 종목일.
  ✅ **읽기 계층에서 수정 완료** — 소비자는 그냥 쓰면 된다(**다시 곱하면 이중조정**).
  🔴 **백테스트 산출물은 전부 수정 «전» 척도로 만들어졌다** — 재검증 시 숫자가 달라진다.
