...
- 1차 구현은 방식 1. 데이터 부족 시 방식 2 fallback.
...
- 워밍업:
  - 1차(엄격): 200 MA + RS 52주(=260일) → 워밍업 260거래일, 검증 가능 ~58일 (표본 부족 위험)
  - 2차(완화): 200 MA + RS 12주(=60일) → 워밍업 200거래일, 검증 가능 ~118일
- 기본 정책: 2차(완화) 채택 + 1차도 보조 산출. RS는 방식 2(단순 12주) 우선, 데이터 확장 시 방식 1로 업그레이드.
