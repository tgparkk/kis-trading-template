...
**재현 대상 (스펙 2.2절, 2026-06-01~2026-06-30, 199종목, TF=3, N=60, D는 버킷, M=60, theta=0.03):**

| 하락폭 버킷 | 표본(봉) | +3% 도달 | −3% 도달 | 상승/하락 비 |
|---|---|---|---|---|
| `no_drop` | 300,667 | 5.55% | 5.14% | 1.080 |
| `1.5-2.5%` | 98,014 | 8.33% | 7.96% | 1.045 |
| `2.5-4.0%` | 61,832 | 13.13% | 12.08% | 1.087 |
| `>=4.0%` | 40,159 | 20.98% | 16.73% | 1.254 |

**허용 오차:** 도달률은 절대 **±0.5%p**, 표본 수는 상대 **±3%**. 임시 SQL은 `hit_close` 조건 없이 `fwd_high`/`fwd_low`만 요구했고 정규장 필터도 없었으므로 완전 일치는 기대하지 않는다. **상승/하락 비의 순서 관계(`no_drop ≈ 2.5-4.0% > 1.5-2.5%`, `>=4.0%`가 최대)는 반드시 보존되어야 한다.** 이것이 스펙의 논지다.
...
def reproduce_spec_table(start: str = "20260601", end: str = "20260630") -> pd.DataFrame:
    ...
    params = LabelParams(timeframe_minutes=3, lookback_min=60,
                         drop_pct=0.0, forward_min=60, theta=0.03)
    ...
    for day in days:
        ...
        for code, g in raw.groupby("stock_code", sort=False):
            ...
            if len(bars) < params.lookback_bars + 2:
                continue
    ...
    agg = all_lab.groupby("bucket").agg(
        n=("hit_up", "size"),
        pct_up=("hit_up", "mean"),
        pct_dn=("hit_down", "mean"),
    ).reset_index()
    agg["pct_up"] = (agg["pct_up"] * 100).round(2)
    agg["pct_dn"] = (agg["pct_dn"] * 100).round(2)
    agg["up_over_dn"] = (agg["pct_up"] / agg["pct_dn"]).round(3)
    return agg.sort_values("pct_up", ascending=False).reset_index(drop=True)
...
`drop_pct=0.0`을 넘겨 모든 봉을 후보로 만들고, 버킷팅은 `drop_pct_actual`로 사후에 한다. 그래야 `no_drop` 베이스라인이 함께 나온다.
...
Expected (허용 오차 내):
```
   bucket      n  pct_up  pct_dn  up_over_dn
   >=4.0%  40159   20.98   16.73       1.254
 2.5-4.0%  61832   13.13   12.08       1.087
1.5-2.5%   98014    8.33    7.96       1.045
  no_drop 300667    5.55    5.14       1.080
```

- [ ] **Step 3: 판정**

**통과 조건 셋을 모두 만족해야 한다.**
1. 각 버킷의 `pct_up`, `pct_dn`이 위 표 대비 절대 ±0.5%p 이내.
2. 각 버킷의 `n`이 위 표 대비 상대 ±3% 이내.
3. `up_over_dn`이 `>=4.0%`에서 최대이고, `1.5-2.5%`가 `no_drop`보다 낮다.

**하나라도 실패하면 멈추고 사용자에게 보고한다.** 코드를 고쳐서 숫자를 맞추려 하지 말 것. 어느 쪽이 맞는지는 사람이 판단해야 한다. 보고 시 다음을 포함한다: 실제 출력 표, 임시 SQL과 정식 파이프라인의 차이 후보(정규장 필터, `bar_count` 미달 봉, `rolling(min_periods=L)`로 인한 초반 봉 제외, 버킷 경계 부등호).

- [ ] **Step 4: 재현 결과를 스펙에 기록**

`docs/superpowers/specs/2026-07-09-intraday-rebound-discovery-design.md` 2.2절 표 아래에 다음을 추가한다 (실제 출력값으로 채운다).

```markdown
**재현 확인 (2026-XX-XX, 정식 파이프라인):** `python -m scripts.discovery.intraday_rebound.reproduce`
결과가 위 표와 허용 오차(도달률 ±0.5%p, 표본 ±3%) 내에서 일치했으며, 상승/하락 비의 순서 관계가 보존되었다.
```
...
- Produces:
  - `_cache/labels_tf{TF}_N{N}_D{D}_M{M}.parquet` — 후보 봉만, 특징 + 라벨 + `trade_date, stock_code, atr14_pct, atr_quintile`
